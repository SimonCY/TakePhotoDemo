...
- (void)setValue:(CGFloat)value shouldCallBack:(BOOL)shouldCallBack {
    if (value != _value) {
        ...
        if (shouldCallBack) {
            ...
            } else if (self.delegate && [self.delegate respondsToSelector:@selector(didChangeValueSCSlider:value:)]) {
...
- (void)callbackTouchEnd:(BOOL)isTouchEnd {
    ...
    } else if (self.delegate && [self.delegate respondsToSelector:@selector(didSCSliderTouchEnd:value:isTouch:)]) {
